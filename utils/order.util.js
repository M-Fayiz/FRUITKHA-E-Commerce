...
const calculateRefundAndAdjustments = (order, product) => {
  ...
  const productGSTshare = Math.round(
    (intialGst / initialSubTotal) * product.TOTAL,
  );
  const productDiscountShare = Math.round(
    (initialDiscount / initialSubTotal) * product.TOTAL,
  );
  ...
  const finalAmount = Math.max(
    0,
    remainingSubTotal + remainingGst + shipping - remainingDiscount,
  );

  return {
    refundAmount: Math.round(refundAmount),
    orderStatus: allProductsReturned
