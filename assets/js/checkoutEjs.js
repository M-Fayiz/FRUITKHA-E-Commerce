...
if (amount > 500) {
  cod.disabled = true;
  info.innerHTML =
    "Cash on Delivery is unavailable for orders above 500. Please choose another payment option";
}
...
document.getElementById("order-form").addEventListener("submit", async (e) => {
  ...
  if (paymentMethod.value === "COD" && Amount > limit) {
    return showToast(
      `Cash on Delivery is unavailable for orders above ₹${limit}. Please choose another payment option`,
      "info",
    );
  }
...
function editAddress(
  ...
  document
    ...
    .addEventListener("submit", async (e) => {
      ...
      const editCustomerName = document
        .getElementById("editCustomerName")
        .value.trim();
      const editAddressName = document
        .getElementById("editAddressName")
        .value.trim();
      ...
      try {
        ...
        const response = await fetch(
          `https://api.postalpincode.in/pincode/${editPincode}`,
        );
        ...
        if (data[0].Status === "Success") {
          ...
          const isValidDistrict = postOffices.some(
            (office) =>
              office.District.toLowerCase() === editCity.toLowerCase(),
          );
          ...
          if (isValidState && isValidDistrict && isValidStreet) {
            ...
            document.getElementById("Result").innerHTML =
              `<p class="text-danger">Validation Failed! Please check the details.</p>`;
          }
        } else {
          document.getElementById("Result").innerHTML =
            `<p class="text-danger">Invalid Pincode or No Data Found.</p>`;
        }
      } catch (error) {
        document.getElementById("Result").innerHTML =
