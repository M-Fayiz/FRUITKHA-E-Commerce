  ...
  document
    ...
    .addEventListener("submit", async (e) => {
      ...
      if (/\s/.test(couponCode)) {
        return showToast(
          "Coupon code should not contain spaces and must be a single word.",
          "error",
        );
      }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const editCouponValueContainer = document.getElementById(
    "editCouponValueContainer",
  );
  ...
  document
    .getElementById("edit-coupon-form")
    .addEventListener("submit", async (e) => {
      e.preventDefault();

      const id = document.getElementById("editCouponId").value;
      const editCouponCode = document
      ...
      const editMinCartValue =
        document.getElementById("editMinCartValue").value;
      const couponMaxDiscoun =
        document.getElementById("couponMaxDiscoun").value;
...
function editCoupon(
  ...
  const handleTypeChange = () => {
    ...
    if (selectedType === "Percentage") {
      ...
      document.getElementById("editCouponValueContainer").style.display =
        "none";
    }
  };
