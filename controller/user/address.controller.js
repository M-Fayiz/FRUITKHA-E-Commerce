...
const addAddres = async (req, res) => {
  try {
    const { Name, adres_name, pincode, place, city, state, user, mark } =
      req.body;
    ...
    if (DATA) {
      return res
        ...
        .json({ success: false, message: httpResponse.ADDRESS_EXISTS });
    } else {
      ...
      if (data) {
        ...
        res
          ...
          .json({ success: true, message: httpResponse.ADDRESS_ADDED });
      } else {
        res
          .status(httpStatusCode.SERVER_ERROR)
          .json({ success: false, message: httpResponse.ADDRESS_ADD_FAILED });
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (!adrsID) {
      return res
        ...
        .json({ success: false, message: httpResponse.ADDRESS_ID_REQUIRED });
    }
    ...
    if (!result) {
      return res
        .status(httpStatusCode.SERVER_ERROR)
        .json({ success: false, message: httpResponse.ADDRESS_DELETE_FAILED });
    }

    res
      .status(httpStatusCode.OK)
      .json({ success: true, message: httpResponse.ADDRESS_DELETED });
  } catch (error) {
...
const editAdddres = async (req, res) => {
  ...
  try {
    ...
    res
      .status(httpStatusCode.OK)
      .json({ success: true, message: httpResponse.ADDRESS_UPDATED });
  } catch (error) {
    console.log(error.message);
    return res
      .status(httpStatusCode.SERVER_ERROR)
