...
const {
  COUPON_STATUS,
  COUPON_TYPE,
} = require("../../constant/status/couponStatus");
...
const coupon = async (req, res) => {
  try {
    ...
    res
      ...
      .render("admin/coupon", { CURRENTpage: "coupon", coupon });
  } catch (error) {
  ...
  try {
    ...
    if (check) {
      return res
        ...
        .json({ success: false, message: httpResponse.COUPON_EXIST });
    }
    ...
    if (data) {
      return res
        ...
        .json({ success: true, message: httpResponse.COUPON_ADDED });
    }
  } catch (error) {
    console.log(error.message);
    return res
  ...
  try {
    ...
    if (!coupon) {
      return res
        ...
        .json({
          success: false,
          message: httpResponse.ITEM_NOT_FOUND("Coupon"),
        });
    }
    ...
    res
      ...
      .json({
        success: true,
        message: httpResponse.UPDATED_SUCCESSFULLY("Coupon", "updated"),
        data: coupon,
      });
  } catch (error) {
...
const couponValidate = async (req, res) => {
  try {
    ...
    if (!result) {
      return res
        .status(httpStatusCode.BAD_REQUEST)
        .json({ success: false, message: httpResponse.INVALID_COUPON });
    }

    if (result.status !== COUPON_STATUS.ACTIVE) {
      return res
        .status(httpStatusCode.BAD_REQUEST)
        .json({ success: false, message: httpResponse.Coupon_not_active });
    }
    ...
    if (now < result.startDate || now > result.endDate) {
      return res
        .status(httpStatusCode.BAD_REQUEST)
        .json({ success: false, message: httpResponse.COUPON_DATE_INVALID });
    }

    if (result.maxUses > 0 && result.uses >= result.maxUses) {
      return res
        .status(httpStatusCode.BAD_REQUEST)
        .json({ success: false, message: httpResponse.COUPON_MAX_REDEEMED });
    }
...
const remove = async (req, res) => {
  try {
    ...
    const couponIndex = user.usedCoupon.findIndex(
      (c) => c.coupon.toString() === couponId.toString(),
    );
    if (couponIndex === -1) {
      return res
        .status(httpStatusCode.BAD_REQUEST)
        .json({
          success: false,
          message: httpResponse.COUPON_HISTORY_NOT_FOUND,
        });
    }
    ...
    if (!cart) {
      return res
        .status(httpStatusCode.BAD_REQUEST)
        .json({ success: false, message: httpResponse.CART_NOT_FOUND_DOT });
    }
  ...
  try {
    ...
    if (result) {
      return res
        ...
        .json({
          success: true,
          message: httpResponse.UPDATED_SUCCESSFULLY("Coupon", "removed"),
        });
    }
    return res
      .status(httpStatusCode.ITEM_NOT_FOUND)
      .json({ success: false, message: httpResponse.ITEM_NOT_FOUND("Coupon") });
  } catch (error) {
    console.log(error.message);
    return res
      .status(httpStatusCode.SERVER_ERROR)
