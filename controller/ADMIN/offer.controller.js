  ...
  try {
    ...
    if (!OfferID) {
      return res
        ...
        .json({ success: false, message: httpResponse.OFFER_ID_REQUIRED });
    }
    ...
    if (!result) {
      return res
        ...
        .json({ success: false, message: httpResponse.OFFER_NOT_FOUND });
    }
    ...
    if (result.categoryId) {
      ...
      if (!updatedCategory) {
        return res
          ...
          .json({
            success: false,
            message: httpResponse.CATEGORY_NOT_FOUND_FOR_OFFER,
          });
      }
      ...
      return res
        ...
        .json({ success: true, message: httpResponse.CATEGORY_OFFER_REMOVED });
    } else if (result.productId) {
      const updatedProduct = await PRODUCT.findByIdAndUpdate(result.productId, {
        $unset: { "Offer.OfferPrice": "", "Offer.OfferId": "" },
      });
      ...
      if (!updatedProduct) {
        return res
          ...
          .json({
            success: false,
            message: httpResponse.PRODUCT_NOT_FOUND_FOR_OFFER,
          });
      }

      return res
        .status(httpStatusCode.OK)
        .json({ success: true, message: httpResponse.PRODUCT_OFFER_REMOVED });
    } else {
      return res
        .status(httpStatusCode.BAD_REQUEST)
        .json({ success: false, message: httpResponse.INVALID_OFFER_TARGET });
    }
  } catch (error) {
  ...
  try {
    ...
    if (categoryID) {
      ...
      if (!CATEGORY) {
        return res
          .status(httpStatusCode.ITEM_NOT_FOUND)
          .json({ success: false, message: httpResponse.CATEGORY_NOT_FOUND });
      }
      ...
      if (duplicateOffer) {
        return res
          ...
          .json({
            success: false,
            message: httpResponse.CATEGORY_OFFER_EXISTS,
          });
      }
      ...
      if (duplicateOffer) {
        return res
          .status(httpStatusCode.BAD_REQUEST)
          .json({ success: false, message: httpResponse.PRODUCT_OFFER_EXISTS });
      }
      savedOffer = new OFFER({
        offer: offerPercentage,
        productId: productId,
    ...
    res
      ...
      .json({ success: true, message: httpResponse.OFFER_ADDED });
  } catch (error) {
    res
      .status(httpStatusCode.SERVER_ERROR)
