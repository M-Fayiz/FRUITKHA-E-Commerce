...
} = require("../../constant/status/orderStatus");
const {
  PAYMENT_METHOD,
  PAYMENT_STATUS,
...
const {
  NOTIFICATION_STATUS,
} = require("../../constant/status/notificationStatus");
...
const OrderStatus = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({ success: true, message: httpResponse.ORDER_STATUS_UPDATED });
  } catch (error) {
  ...
  try {
    ...
    if (!User) {
      return res
        ...
        .json({ success: false, message: httpResponse.NOT_LOGGED_IN });
    }
    ...
    const allProductsCancelled = order.Products.every(
      (p) => p.status === ORDER_STATUS.CANCELLED,
    );
  ...
  try {
    ...
    if (selected === RETURN_ACTION.APPROVE) {
      if (itemId) {
        ...
        const product = order.Products[productIndex];
        product.return.adminStatus = {
          status: true,
          response: RETURN_ADMIN_RESPONSE.APPROVED,
        };
        ...
        await sendNotification(
          req.session.user,
          "Product approved and marked as returned.",
          NOTIFICATION_STATUS.SUCCESS,
        );
        return res
          .status(httpStatusCode.OK)
          .json({
            success: true,
            message: httpResponse.PRODUCT_MARKED_RETURNED,
          });
      } else if (!itemId) {
        order.Products.forEach((product) => {
          if (product.status !== ORDER_STATUS.CANCELLED)
        ...
        return res
          .status(httpStatusCode.OK)
          .json({ success: true, message: httpResponse.ORDER_MARKED_RETURNED });
      }
    }
    ...
    if (selected === RETURN_ACTION.CANCEL) {
      if (itemId) {
          ...
          { _id: orderId, "Products._id": itemId },
          {
            $set: {
              "Products.$.return.adminStatus": {
                status: false,
                response: RETURN_ADMIN_RESPONSE.REJECTED,
              },
            },
          },
        );
        ...
        return res
          ...
          .json({
            success: true,
            message: httpResponse.PRODUCT_RETURN_REJECTED,
          });
      } else {
        await ORDER.findOneAndUpdate(
          { _id: orderId },
        ...
        await sendNotification(
          req.session.user,
          "Order return request rejected.",
          NOTIFICATION_STATUS.ERROR,
        );
        return res
          .status(httpStatusCode.OK)
          .json({ success: true, message: httpResponse.ORDER_RETURN_REJECTED });
      }
    }
  } catch (error) {
    ...
    return res
      .status(httpStatusCode.SERVER_ERROR)
      .json({ success: false, message: httpResponse.RETURN_HANDLE_ERROR });
  }
};
module.exports = {
  order,
